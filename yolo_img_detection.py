...
class ImageDetection(object):
    def __init__(self , input_image , output_image):
        ...
        self.vs = cv2.VideoCapture(input_image)
        self.vs.set(cv2.CAP_PROP_BUFFERSIZE, 2)

        self.confThreshold = 0.2
        self.nmsThreshold = 0.0
        self.inpWidth = 1024
        self.inpHeight = 1024
        self.outputFrame = None
        self.outputFile = output_image
    ...
    def postprocess(self ,frame, outs):
        ...
        for i in indices:
            ...
            #cropped = frame[left:left+height ,top:top+width]
            #cropped = frame[left:left+width ,top:top+height]
            #image[top:((w-down)+top), right:((h-left)+right)]
            #cv2.imwrite("pothole"+str(box[0])+"thumbnail.png", cropped)
        return nb
    ...
    def get_frame(self):
        ...
        cv2.putText(frame, "Number of Potholes : " + str(round(nb, 2)), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 100, 0) , 2)
